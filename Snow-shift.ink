...
// МЕСТОПОЛОЖЕНИЕ МАЛЫШЕЙ
// "home", "club", "greenhouse"
VAR andreyka_location = "home"
VAR sonya_tonya_location = "home"
VAR kirillka_location = "home"

// ФЛАГИ ДЛЯ ДИАЛОГОВ И МЕХАНИК
VAR kirillka_hijack_event_triggered = false // Угнал ли Кирилка Шнекоротор?
VAR kirillka_hijack_dialogue_seen = false   // Поговорили ли про альтернативу угону?
...
VAR club_repair_dialogue_seen = false // Видел ли игрок диалог и выбор про починку дрона в Клубе?
VAR greenhouse_repair_dialogue_seen = false // Видел ли игрок диалог и выбор про починку агродрона?

// -----------------------------------------------------------------
// 5. ПЕРЕМЕННЫЕ ПОДРОСТКОВ (ЛОКАЦИЯ И ИНВЕНТАРЬ)
// -----------------------------------------------------------------
// Местоположение персонажей. В целом -- пока что бессмысленная вещь. Но вдруг я однажды захочу усложнить механику и оставлять подростков там, где они закончили час?
...
// 2. В локации есть нужный малыш/и в состоянии "useful".
...
// Здесь происходят все автоматические события "в начале часа".
=== upkeep_phase ===
...
=== report_phase ===
// --- ЭТАП 1: ПРОВЕРКА СЦЕНЫ РАСЧИСТКИ ЗАВАЛА ---
...
# clear
{
    ...
    - game_result == "2_звезды":
        ...
        # clear
        Гул спасательного дирижабля становится все громче. Надя с облегчением выдыхает. Они не просто продержались — они смогли навести порядок.
...
Маша: Он просто одержим этой идеей. Мы пытались объяснить, что это опасно, но он никого не слушает. Считает, что только он может их спасти.
...
Артем: Наконец-то. Я уже начал переживать за Соню и Тоню. Значит, если девочки будут здоровы, я смогу их туда отвести, а потом окончательно забыть о беспокойстве за них?
...
Надя: Хм... Занять их делом, которое еще и пользу принесет? Это отличная мысль! Хорошо, команда, принимаем к сведению: если будет возможность, нужно доставить одну запчасть в Клуб. Это может решить сразу две проблемы.
...
=== kirillka_bored_dialogue ===
# clear
Маша: Надя, у нас тут... дипломатический кризис. Кирилка в Клубе сидеть отказывается. Говорит, что тут нет ничего 'для серьезных агрономов' и что ему здесь скучно.
...
+ [Сказать, чтобы не выдумывал. Дисциплина прежде всего.]
    Надя: Маша, скажите ему, чтобы сидел тихо. Сейчас не до капризов и не до апельсинов. Пусть там ну хоть просто посидит с ребятами, хотя бы не будет доставлять проблем.
    ~ kirillka_bored_dialogue_seen = true
    -> return_from_bored_dialogue

+ [Расспросить его. Может, он в чем-то прав?]
    Надя: Погоди, Игнат. А что если он прав? Артем, ты у нас во всех этих апельсинах разбираешься. Не напортачит он там?
    Артем: Сложно сказать, не видя его 'апельсинчиков'. Но он действительно разбирается в агрономии, и в работающей Теплице от него будет больше пользы, чем в Клубе, где он сидит из-под палки. И знаешь что еще... Агродрон в Теплице ведь тоже сломан. Если мы доставим туда запчасти, Кирилка сможет с ним разобраться. Это его любимая игрушка, он с детства его схемы изучал.
    Надя: Вот как... То есть, мы можем спасти эти его апельсины и заодно починить ценное оборудование? Хорошо. Это риск, но, возможно, оправданный. Даю добро на агро-практику!
    ~ kirillka_bored_dialogue_seen = true
    ~ kirillka_greenhouse_option_unlocked = true
    -> return_from_bored_dialogue
...
- else:
    ...
    - else:
        { kirillka_activity == "naughty": // <-- ДОБАВЛЕНО ЯВНОЕ УСЛОВИЕ
            Игнат: Надя, он это сделал! Кирилка угнал шнекоротор! Следы ведут в сторону Теплостанции. Говорит, что-то про свои апельсины! #У
        }
    }
}
...
=== scene_expansion_success ===
#Location: Расчистка завала
С ревом и скрежетом металла шнекоротор вгрызается в плотный снег. Огромные фрезы перемалывают лед и отбрасывают снежную крошку на десятки метров. Еще мгновение — и стена снега, отрезавшая вас от другой части поселка, рушится. Путь расчищен. Доступна новая зона!
+ [Продолжить]
    -> report_phase // Возвращаемся к отчету

=== scene_greenhouse_fixed ===
#Location: Теплица
Маша: Есть! Системы Теплицы перезапущены! Наши персики и апельсинчики спасены! #Р
+ [Отлично!]
    -> report_phase // Возвращаемся к отчету
...
Маша (по рации): О, ты как раз вовремя! Они от него не отходят. Говорят, что знают, как его починить.
...
+ [Оставить им запчасть. Пусть попробуют!]
    Надя: Хорошо, Маша. Оставляю детали здесь. Пусть действуют, но под вашим с Игнатом удаленным присмотром! #Р
...
Артем: Погоди, Кирилка! Ты уверен, что справишься? Это сложная система.
...
Статус победы: выживание, 1 звезда! Поздравляем!
-> END
...
Надя: (позволяет себе слабую улыбку.) Это было больше, чем просто выживание!
Статус победы: стабилизация, 2 звезды! Поздравляем!
-> END
...
Надя: (оглядывая Артема, Игната и Машу) Мы сделали это вместе, ребята.
Статус победы: полный успех, 3 звезды. Поздравляем!
-> END
