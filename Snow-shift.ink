...
{ expansion_event_happened:
    ~ expansion_event_happened = false // Сбрасываем флаг, чтобы показать сцену только один раз
    -> scene_expansion_success         // Переходим к сцене
}

{ greenhouse_just_fixed:
    ~ greenhouse_just_fixed = false // Сбрасываем флаг
    -> scene_greenhouse_fixed      // Переходим к сцене с Теплицей
}
...
= assign_ignat
    ...
    + {zone_5_accessible} [Чинить Медпункт (5)]
        ...
        -> assignment_loop
    + {zone_4_accessible} [Чинить Теплицу (4)]
...
# clear
{
    ...
    // -------------------------------------------------
    - game_result == "поражение_холод":
        #Location: Кризисный центр (Тревога)
